  ...
  awful.prompt.run({
    ...
    function ()
      if t.name == before then
        if awful.tag.getproperty(t, "initial")
          then shifty.del(t)
        end
      else
...
function shifty.send_next()
  send(1)
end
function shifty.send_prev()
  send(-1)
end
...
function shifty.tagtoscr(scr, t)
  ...
  if not scr or scr < 1 or scr > capi.screen.count() then
    return
  end
  -- tag to move
...
function set(t, args)
  ...
  local props = {
    layout = select({
      args.layout, preset_layout,
      awful.tag.getproperty(t, "layout"),
      shifty.config.defaults.layout,
    ...
    nmaster = select({
      ...
      shifty.config.defaults.nmaster,
      1
    }),
    ncol = select({
      args.ncol,
    ...
    leave_kills = select({
      args.leave_kills, preset.leave_kills,
    ...
    -- FIXME: This and trick is silly
    icon = select({
      args.icon and args.icon,
    ...
    overload_keys = select({
      args.overload_keys,
      preset.overload_keys,
  ...
  local rel_index = args.rel_index or preset.rel_index or shifty.config.defaults.rel_index
  local sel = awful.tag.selected(scr)
  ...
  for prop, val in pairs(props) do
    awful.tag.setproperty(t, prop, val)
  end
...
function shifty.add(args)
  ...
  if not (awful.tag.getproperty(t, "nopopup") or args.noswitch) or #awful.tag.gettags(awful.tag.getscreen(t)) == 1 then
...
function match(c)
  ...
  -- TODO: Refactor this shit, this is just... monolitic..
    ...
    if a.match then
      ...
      if not matched then
        for k, w in ipairs(a.match) do
          matched = (cls and cls:find(w)) or (inst and inst:find(w)) or (name and name:find(w)) or (role and role:find(w)) or (typ and typ:find(w))
          if matched then
            break
          end
        end
      ...
      -- set attributes
      -- TODO: Another monolitic piece of shit
      if matched then
        if a.screen then
        ...
        if a.geometry ~=nil then
          geom = {x = a.geometry[1], y = a.geometry[2], width = a.geometry[3], height = a.geometry[4]}
        end
        if a.float ~= nil then
          float = a.float
        end
        ...
        if a.nofocus ~= nil then
          nofocus = a.nofocus
        end
        if a.keys ~= nil then
          keys = awful.util.table.join(keys, a.keys)
        end
        if a.hidden ~= nil then
          c.hidden = a.hidden
        end
  ...
  if not target_tag_names or #target_tag_names == 0 then
    ...
    elseif #sel > 0 then
      for i, t in ipairs(sel) do
        ...
        if intrusive or not (awful.tag.getproperty(t, "exclusive") or (mc and mc >= #t:clients())) then
  ...
  if #target_tag_names > 0 and #target_tags == 0 then
    ...
    for i, tn in ipairs(target_tag_names) do
      ...
      for j, t in ipairs(name2tags(tn, target_screen) or
        ...
        if intrusive or not (mc and (((#t:clients() >= mc) and not tagged) or (#t:clients() > mc))) or intrusive then
      ...
      if #res == 0 then
        ...
        shifty.add({name = tn, noswitch = true, matched = true}))
      else
  ...
  if not (nofocus or c.hidden or c.minimized) then
    ...
    if (target and target ~= sel) and (awful.tag.getproperty(target, "nopopup") or nopopup)  then
...
function sweep()
  for s = 1, capi.screen.count() do
    for i, t in ipairs(awful.tag.gettags(s)) do
      ...
      if #clients == sticky then
        if awful.tag.getproperty(t, "used") and not awful.tag.getproperty(t, "persist") then
          if awful.tag.getproperty(t, "deserted") or not awful.tag.getproperty(t, "leave_kills") then
            ...
            if awful.tag.getproperty(t, "visited") and not t.selected then
...
function shifty.getpos(pos, scr_arg)
  ...
  if #existing > 0 then
    ...
    if not selected then
      for _, tag in pairs(existing) do
        if awful.tag.getscreen(tag) == scr then
          return tag
        end
      end
    ...
    if (scr_arg == nil or awful.tag.getscreen(tag) == scr_arg) and not tag.selected then
      return tag
    end
    until i == selected
    ...
    if scr_arg == nil or awful.tag.getscreen(existing[selected]) == scr then
  ...
  for i, j in pairs(shifty.config.tags) do
    ...
    if type(tag_scr) ~= 'table' then
      tag_scr = {tag_scr}
    end

    if j.position == pos and awful.util.table.hasitem(tag_scr, scr) then
      return shifty.add({name = i, position = pos, noswitch = not switch})
    end
  end

  -- not existing, not preconfigured
  return shifty.add({position = pos, rename = pos .. ':', no_selectall = true, noswitch = not switch})
end
  ...
  for i, j in pairs(shifty.config.tags) do
    ...
    for _, s in pairs(scr) do
      if j.init and (s <= numscr) then
        shifty.add({name = i, persist = true, screen = s, layout = j.layout, mwfact = j.mwfact})
      end
    end
  end
end
...
function tagkeys(s)
  ...
  local keys = awful.tag.getproperty(sel, "keys") or shifty.config.globalkeys
  if keys and sel.selected then
    capi.root.keys(keys)
  end
end
